...
import { useChangeUserAvatar, useRemoveUserAvatar, useSessionUser } from "@/services/users.service";
import { initialToastState, toastReducer, ToastState } from "@/reducers/toast.reducer";
...
export const useAvatarForm = (): UseAvatarFormReturn => {
  ...
  const { mutate: submitChangeAvatar, isPending: isSubmittingChangeAvatar } = useChangeUserAvatar();
  const { mutate: submitRemoveUserAvatar, isPending: isSubmittingRemoveUserAvatar } = useRemoveUserAvatar();
  ...
  const handleAvatarSubmit = () => {
    ...
    submitChangeAvatar(
      { image: avatarSourceImage, userId } as any,
      {
        onSuccess: () => {
          dispatchToast({ type: "SHOW_SUCCESS", message: "Avatar updated successfully!" });
        },
        onError: () => {
          dispatchToast({ type: "SHOW_ERROR", message: "Failed to update avatar." });
        },
      }
    );
  };
  ...
  const handleDeleteAvatar = () => {
    ...
    submitRemoveUserAvatar(
      { userId } as any,
      {
        onSuccess: () => {
          setAvatarSourceImage(null);
          setInitialImage(null);
          dispatchToast({ type: "SHOW_SUCCESS", message: "Avatar deleted successfully!" });
        },
        onError: () => {
          dispatchToast({ type: "SHOW_ERROR", message: "Failed to delete avatar." });
        },
      }
    );
  };
