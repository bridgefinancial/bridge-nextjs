...
2. **Molecules**: Combinations of atoms that form more complex components, like form fields or card layouts. These components are functional and reusable across the application.
    ...
    **Example: Form Field Component**
    ...
    const FormField: React.FC<FormFieldProps> = ({ label, type, placeholder, description, value, onChange }) => (
    ...
    const MainHeader: React.FC = () => (
      <header className="flex justify-between items-center p-4 bg-white shadow-md">
        ...
        <div className="flex items-center gap-4">
          <FormField label="Search" type="text" value="" onChange={() => {}} placeholder="Search..." />
...
const UserProfile: React.FC<UserProfileProps> = ({ userId }) => {
  ...
  return (
    <div className="p-4 border rounded-lg">
      ...
      <PrimaryButton onClick={() => console.log('Button clicked!')}>Message</PrimaryButton>
    </div>
  );
};
